...
struct RandomGenerator
{
    ...
    RandomGenerator(ad::Size size = 0,
...
struct RandomGenerator<std::pair<Val, Val>>
{
    ...
    auto operator()()
    {
        return std::make_pair(mUd(mMt), mUd(mMt));
    }
...
template <class Val, class Reverse = False_,
    class Compare = std::less<Val>>
struct SortedGenerator
{
    ...
    SortedGenerator(ad::Size size = 1,
        RandomType min = std::numeric_limits<RandomType>::min(),
        RandomType max = std::numeric_limits<RandomType>::max())
        : mVec(size), mAt(0)
    {
