...
struct TmpBuff {
    ...
    TmpBuff() : mMem(nullptr), mCapacity(0), mSize(0) {}

    TmpBuff(PtrDiff sz) : mMem(nullptr), mCapacity(0), mSize(0) {
    ...
    ~TmpBuff() { std::return_temporary_buffer(mMem); }

    PtrDiff capacity() const { return mCapacity; }

    PtrDiff size() const { return mSize; }

    Iterator begin() { return mMem; }

    Iterator end() { return mMem + size(); }

    Void init() { std::uninitialized_fill(begin(), end(), T()); }
...
class Pair
  ...
  public:
    ...
    Pair(
        ...
        : FirstBase(
              pc, std::move(firstArgs), std::index_sequence_for<Args1...>()),
          SecondBase(
              pc, std::move(secondArgs), std::index_sequence_for<Args2...>()) {}
    ...
    PairType& operator=(std::conditional_t<
                        ...
                        const PairType&, const _utility::DeadClass&>
                            other) {
    ...
    FirstType& first() & { return FirstBase::get(); }

    const FirstType& first() const& { return FirstBase::get(); }

    FirstType&& first() && { return std::move(FirstBase::get()); }

    const FirstType&& first() const&& { return std::move(FirstBase::get()); }

    SecondType& second() & { return SecondBase::get(); }

    const SecondType& second() const& { return SecondBase::get(); }

    SecondType&& second() && { return std::move(SecondBase::get()); }

    const SecondType&& second() const&& { return std::move(SecondBase::get()); }
