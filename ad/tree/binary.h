...
class BinaryTreeNode {
  ...
  public:
    ...
    NodePtr leftChild() { return mLeftChild; }

    NodePtr rightChild() { return mRightChild; }

    Void clear() { mParent = mLeftChild = mRightChild = nullptr; }
...
class BinaryTreeVisitor {
  ...
  public:
    ...
    BinaryTreeVisitor() : mNode(nullptr) {}

    BinaryTreeVisitor(const VisitorType& visitor) : mNode(visitor.node()) {}
    ...
    VisitorType first() const {
        return VisitorType(
            mNode->leftChild() ? mNode->leftChild() : mNode->rightChild());
    }

    VisitorType last() const {
        return VisitorType(
            mNode->rightChild() ? mNode->rightChild() : mNode->leftChild());
    }
    ...
    VisitorType leftChild() const { return VisitorType(mNode->leftChild()); }

    VisitorType rightChild() const { return VisitorType(mNode->rightChild()); }
...
class BinaryTreeConstVisitor {
  ...
  public:
    ...
    BinaryTreeConstVisitor() : mNode(nullptr) {}
...
class BinaryTreeBase {
  ...
  protected:
    ...
    BinaryTreeBase() : mNodeAl(), mRoot(nullptr) {}
    ...
    Void destroy(NodePtr node) {
        ...
        for (auto parent = postBegin(Visitor(node)); parent != parentEnd;
             ++parent) {
...
class BinaryTree : public _binary::BinaryTreeBase<Val, Alloc> {
  ...
  public:
    ...
    explicit BinaryTree(const AllocatorType& alloc) : Base(alloc) {}
    ...
    explicit BinaryTree(
        ValueType&& val, const AllocatorType& alloc = AllocatorType())
        : Base(alloc) {
    ...
    BinaryTree(const TreeType& tree) : Base(tree) {}

    BinaryTree(TreeType&& tree) : Base(std::move(tree)) {}
    ...
    TreeType spliceLeft(ConstVisitor parent, TreeType& tree) {
        return TreeType(
            Base::nodeAlloc(),
            Base::insertLeft(
                parent.node(), tree.Base::remove(tree.rootNode())));
    }
    ...
    TreeType
        spliceLeft(ConstVisitor parent, TreeType& tree, ConstVisitor root) {
        return TreeType(
            Base::nodeAlloc(),
...
Bool operator==(
    ...
    return std::equal(
        preBegin(l.croot()), preEnd(l.root()), preBegin(r.croot()),
        preEnd(r.root()));
}
