...
public:
    ...
    VisitorType last() const { return VisitorType(mTree, (mIdx << 1) + 1); }

    VisitorType left() const {
        return !(mIdx & 1) || (mIdx == 1) ? VisitorType(nullptr, 0)
    ...
    VisitorType leftChild() const { return first(); }

    VisitorType rightChild() const { return last(); }
...
public:
    ...
    CBTreeConstVisitor() : mTree(nullptr), mIdx(0) {}
    ...
    ConstVisitorType left() const {
        return !(mIdx & 1) || (mIdx == 1) ? ConstVisitorType(nullptr, 0)
    ...
    ConstVisitorType leftChild() const { return first(); }

    ConstVisitorType rightChild() const { return last(); }
...
public:
    ...
    CompleteBinaryTree() : mVec() {}

    explicit CompleteBinaryTree(const AllocatorType& alloc) : mVec(alloc) {}

    explicit CompleteBinaryTree(
        SizeType count, ConstReference val = ValueType(),
    ...
    CompleteBinaryTree(const TreeType& tree) : mVec(tree.mVec) {}

    CompleteBinaryTree(TreeType&& tree) : mVec(std::move(tree.mVec)) {}
    ...
    SizeType size() const { return mVec.size(); }

    SizeType maxSize() const { return mVec.max_size(); }

    SizeType max_size() const { return maxSize(); }

    Visitor root() { return Visitor(this, 1); }

    ConstVisitor root() const { return ConstVisitor(this, 1); }

    ConstVisitor croot() const { return root(); }

    AllocatorType getAllocator() const { return mVec.get_allocator(); }

    AllocatorType get_allocator() const { return getAllocator(); }

    Void resize(SizeType count, ConstReference val) { mVec.resize(count, val); }
    ...
    Void pushBack(ConstReference val) { mVec.push_back(val); }
    ...
    Void popBack() { mVec.pop_back(); }

    Reference operator[](SizeType pos) { return mVec[pos]; }
